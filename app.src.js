...
const App = {
  ...
  _renderWorkbenchClaim() {
    const el = document.getElementById('workbenchBody');
    if (!el) return;
    el.innerHTML = `
      <div class="card">
        <div class="card-header">① 认领你的艺人主页</div>
        <div class="card-body">
          <p class="text-secondary text-sm" style="margin-bottom:12px;">搜索并认领你的艺人主页（每个主页仅可被一个账号认领，用于确权）。</p>
          <div class="flex gap-2">
            <input class="form-input" id="wbClaimInput" placeholder="输入你的艺人姓名，如 肖战" onkeydown="if(event.key==='Enter')App.artistWorkbenchSearch()">
    ...
    try {
      ...
      el.innerHTML = list.map(a => `
        <div class="flex justify-between items-center" style="padding:10px 0;border-bottom:1px solid var(--border-light);">
          ...
          <button class="btn btn-primary btn-sm" type="button" onclick="App.artistWorkbenchClaim(${a.id})">认领</button>
        </div>`).join('');
    } catch (e) {
  ...
  async artistWorkbenchClaim(artistId) {
    try {
      const r = await this.api('/artist/claim', { method: 'POST', body: JSON.stringify({ artist_id: artistId }) });
      this.showToast(r.message || '认领成功', 'success');
      const d = await this.api('/artist/me');
      this._renderWorkbenchDashboard(d);
    } catch (e) {
