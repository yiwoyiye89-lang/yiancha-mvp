...
const App = {
  ...
  state: {
    ...
    affId: null, affProfile: null, // AFF 粉丝力三维度缓存（按艺人）
  },
  ...
  route() {
    ...
    if (hash.startsWith('artist/')) {
      ...
      const id = hash.replace('graph/', '');
      this.renderGraphPage(container, id);
    } else if (hash === 'monitoring') {
      this.renderMonitoringPage(container);
    } else if (hash === 'workbench') {
      this.renderArtistWorkbench(container);
    } else if (hash === 'aff') {
  ...
  async renderUserArea() {
    ...
    if (this.state.user && this.state.token) {
      ...
      const roleName = { individual: '个人', brand: '品牌方', mcn: 'MCN', agency: '代理', production: '制作', government: '政府/媒体', artist: '艺人', admin: '管理员' }[role] || '个人';
      ...
      if (role === 'brand') {
        ...
        roleActions = `<button class="btn btn-ghost btn-xs" type="button" onclick="App.navigate('workbench')">我的工作台</button>`;
      }
      // P0-2/3/4 通用入口
      const moreActions = `
  ...
  // ===== 艺人工作台（supply-side 核心）=====
  async renderArtistWorkbench(container) {
    if (!this.state.token) {
      container.innerHTML = `<div class="page-content" style="max-width:760px;margin:0 auto;padding:var(--space-8) var(--space-6);text-align:center;">
        <div style="font-size:48px;margin-bottom:12px;">🎤</div>
        <h1 style="font-size:26px;font-weight:800;">我的艺人工作台</h1>
        <p class="text-secondary" style="margin:12px 0 24px;">艺人可认领自己的主页，查看风险档案、代言战报、品牌需求与监测动态。</p>
        <button class="btn btn-primary" onclick="App.openAuth()">登录后进入</button>
      </div>`;
      return;
    }
    container.innerHTML = `<div class="page-content" style="max-width:1200px;margin:0 auto;padding:var(--space-8) var(--space-6);">
      <div class="flex justify-between items-center mb-6">
        <div>
          <h1 style="font-size:28px;font-weight:800;letter-spacing:-1px;">🎤 我的艺人工作台</h1>
          <p class="text-secondary" style="margin-top:6px;">认领你的艺人主页，掌握自己的风险档案、代言战报与品牌需求。</p>
        </div>
      </div>
      <div id="workbenchBody"><div class="loading-overlay"><div class="spinner"></div></div></div>
    </div>`;
    try {
      const claims = await this.api('/artist/claims');
      const items = (claims.items || []);
      const ownerApproved = items.find(c => c.claim_type === 'owner' && c.status === 'approved');
      if (ownerApproved) {
        const d = await this.api('/artist/me');
        this._renderWorkbenchDashboard(d);
      } else {
        this._renderWorkbenchClaim(items);
      }
    } catch (e) {
      const el = document.getElementById('workbenchBody');
      if (el) el.innerHTML = `<div class="text-secondary text-sm">加载失败：${e && e.message ? e.message : '未知错误'}</div>`;
    }
  },

  _renderWorkbenchClaim(claims = []) {
    const el = document.getElementById('workbenchBody');
    if (!el) return;
    const statusBadge = (s) => s === 'approved' ? '<span class="tag tag-risk-safe">已通过</span>'
      : s === 'pending' ? '<span class="tag tag-risk-mid">审核中</span>'
      : '<span class="tag tag-risk-high">已驳回</span>';
    const typeLabel = { owner: '本人主页', manager: '经纪管理', mcn: 'MCN 管理', agency: '代理管理' };
    const claimList = claims.length ? `
      <div class="card" style="margin-bottom:16px;">
        <div class="card-header">我的认领申请</div>
        <div class="card-body">
          ${claims.map(c => `<div class="flex justify-between items-center" style="padding:8px 0;border-bottom:1px solid var(--border-light);">
            <div><b>${c.artist_name || ('艺人#' + c.artist_id)}</b> <span class="text-tertiary text-xs">· ${typeLabel[c.claim_type] || c.claim_type}</span></div>
            <div>${statusBadge(c.status)} ${c.status === 'rejected' && c.reject_reason ? `<span class="text-tertiary text-xs">（${c.reject_reason}）</span>` : ''}</div>
          </div>`).join('')}
        </div>
      </div>` : '';
    el.innerHTML = `
      ${claimList}
      <div class="card">
        <div class="card-header">① 提交认领申请</div>
        <div class="card-body">
          <p class="text-secondary text-sm" style="margin-bottom:12px;">认领需运营审核（防冒领）。可选择认领类型：本人主页确权，或由经纪/MCN/代理替艺人管理主页。</p>
          <div class="flex gap-2" style="margin-bottom:10px;">
            <select class="form-input" id="wbClaimType" style="max-width:180px;">
              <option value="owner">本人主页</option>
              <option value="manager">经纪管理</option>
              <option value="mcn">MCN 管理</option>
              <option value="agency">代理管理</option>
            </select>
          </div>
          <div class="flex gap-2">
            <input class="form-input" id="wbClaimInput" placeholder="输入艺人姓名，如 肖战" onkeydown="if(event.key==='Enter')App.artistWorkbenchSearch()">
            <button class="btn btn-outline btn-sm" type="button" onclick="App.artistWorkbenchSearch()">搜索</button>
          </div>
          <div id="wbClaimResults" style="margin-top:12px;"></div>
        </div>
      </div>`;
  },

  async artistWorkbenchSearch() {
    const el = document.getElementById('wbClaimResults');
    if (!el) return;
    const inp = document.getElementById('wbClaimInput');
    const kw = inp ? inp.value.trim() : '';
    if (!kw) { this.showToast('请输入艺人姓名', 'warn'); return; }
    el.innerHTML = `<div class="loading-overlay"><div class="spinner"></div></div>`;
    try {
      const list = await this.api('/artists/search?keyword=' + encodeURIComponent(kw) + '&limit=20');
      if (!list || !list.length) {
        el.innerHTML = `<div class="text-tertiary text-sm" style="padding:12px 0;">未找到匹配艺人，请确认姓名或联系运营录入。</div>`;
        return;
      }
      el.innerHTML = list.map(a => `
        <div class="flex justify-between items-center" style="padding:10px 0;border-bottom:1px solid var(--border-light);">
          <div>
            <b>${a.name}</b>
            <span class="text-tertiary text-xs"> · ${a.heat_level || '-'} · ${a.risk_level || '-'}</span>
            ${a.agency ? `<div class="text-secondary text-xs">${a.agency}</div>` : ''}
          </div>
          <button class="btn btn-primary btn-sm" type="button" onclick="App.artistWorkbenchClaim(${a.id}, document.getElementById('wbClaimType').value)">认领</button>
        </div>`).join('');
    } catch (e) {
      el.innerHTML = `<div class="text-secondary text-sm">搜索失败</div>`;
    }
  },

  async artistWorkbenchClaim(artistId, claimType = 'owner') {
    try {
      const r = await this.api('/artist/claim', { method: 'POST', body: JSON.stringify({ artist_id: artistId, claim_type: claimType }) });
      this.showToast(r.message || '已提交申请', 'success');
      const claims = await this.api('/artist/claims');
      this._renderWorkbenchClaim(claims.items || []);
    } catch (e) {
      this.showToast((e && e.message) || '认领失败', 'error');
    }
  },

  async _renderWorkbenchDashboard(d) {
    const el = document.getElementById('workbenchBody');
    if (!el) return;
    const a = d.artist || {};
    const riskColor = (a.risk_level === '高风险') ? 'var(--risk-extreme)' : (a.risk_level === '中风险' ? 'var(--risk-high)' : 'var(--risk-safe)');
    const riskSummary = a.risk_summary ? `<div class="card" style="margin-top:16px;"><div class="card-header">🛡️ 风险摘要</div><div class="card-body text-sm" style="line-height:1.8;">${a.risk_summary}</div></div>` : '';
    const endorse = d.endorsements || {};
    const inbound = (d.inbound_brand_requirements || {}).items || [];
    const mon = d.monitoring || {};

    const endorseHtml = (endorse.items && endorse.items.length)
      ? endorse.items.map(e => `<tr>
          <td>${e.brand || '-'}</td>
          <td>${e.category || '-'}</td>
          <td>${e.period || '-'}</td>
          <td>${e.gmv_wan != null ? (e.gmv_wan + ' 万' + (e.gmv_is_lower_bound ? '（下限）' : '')) : '-'}</td>
          <td>${e.verified ? '✓已核对' : '待核对'}</td>
        </tr>`).join('')
      : `<tr><td colspan="5" class="text-tertiary text-sm">暂无代言战报数据</td></tr>`;

    const inboundHtml = inbound.length
      ? inbound.map(b => `<div class="flex justify-between items-center" style="padding:10px 0;border-bottom:1px solid var(--border-light);">
          <div><b>${b.industry || '品牌需求'}</b> <span class="text-tertiary text-xs">· ${b.budget_range || ''} · ${b.tone || ''}</span>${b.target_audience ? `<div class="text-secondary text-xs">${b.target_audience}</div>` : ''}</div>
          <span class="tag tag-risk-safe">想找你</span>
        </div>`).join('')
      : `<div class="text-tertiary text-sm" style="padding:12px 0;">暂无品牌将你设为匹配候选，完善风险档案与代表作可提升命中率。</div>`;

    const alertHtml = (mon.recent_alerts && mon.recent_alerts.length)
      ? mon.recent_alerts.map(al => `<div style="padding:8px 0;border-bottom:1px solid var(--border-light);">
          <span class="tag ${al.severity === 'high' || al.severity === 'critical' ? 'tag-risk-high' : 'tag-risk-safe'}">${this.monitorSeverityText(al.severity)}</span>
          <span class="text-sm" style="margin-left:8px;">${al.message || ''}</span>
          ${al.is_read ? '' : '<span class="tag tag-risk-mid" style="margin-left:8px;">未读</span>'}
          <div class="text-tertiary text-xs">${al.created_at || ''}</div>
        </div>`).join('')
      : `<div class="text-tertiary text-sm" style="padding:12px 0;">暂无风险动态</div>`;

    const mArt = (d.managed_artists && d.managed_artists.items) || [];
    let managedHtml = mArt.length ? `
      <div class="card" style="margin-top:16px;">
        <div class="card-header">🗂️ 我管理的艺人 <span class="tag" style="background:#F0F9FF;color:#0369A1;">${mArt.length} 位</span></div>
        <div class="card-body"><div style="display:flex;flex-wrap:wrap;gap:12px;">
          ${mArt.map(x => `
            <div style="border:1px solid var(--border-light);border-radius:12px;padding:12px;min-width:180px;">
              <div style="font-weight:700;">${x.name}</div>
              <div class="text-tertiary text-xs">${x.heat_level || '-'} · ${x.risk_level || '-'} · ${x.agency || '无经纪'}</div>
              <div style="margin-top:6px;"><span class="tag tag-risk-safe">${x.claim_type_label || x.claim_type}</span></div>
              <button class="btn btn-ghost btn-xs" type="button" onclick="App.navigate('artist/${x.id}')">查看主页</button>
            </div>`).join('')}
        </div></div>
      </div>` : '';
    // 管理方矩阵增强：批量管理 + 同公司风险传导（优先用 /artist/matrix 富数据）
    try {
      const mxResp = await this.api('/artist/matrix');
      const mx = (mxResp && mxResp.items) || [];
      if (mx.length) managedHtml = this._renderMatrixCard(mx);
    } catch (e) { /* 矩阵接口不可用时回退到简单列表 */ }


    el.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:16px;margin-bottom:24px;">
        ${this.monitorStatCard('热度评级', a.heat_level || '-', 'var(--primary)')}
        ${this.monitorStatCard('风险等级', a.risk_level || '-', riskColor)}
        ${this.monitorStatCard('风险总分', a.risk_score != null ? a.risk_score : '-', riskColor)}
        ${this.monitorStatCard('被监测机构', mon.watcher_count || 0, 'var(--accent-purple)')}
        ${this.monitorStatCard('未读预警', mon.unread_alerts || 0, (mon.unread_alerts || 0) > 0 ? 'var(--risk-high)' : 'var(--primary)')}
      </div>

      <div class="grid-2">
        <div class="card">
          <div class="card-header">📊 我的风险档案</div>
          <div class="card-body text-sm" style="line-height:1.9;">
            <div>经纪公司：<b>${a.agency || '-'}</b></div>
            <div>人设标签：${a.persona_tags || '-'}</div>
            <div>代表作：${a.masterpieces || '-'}</div>
            <div>商务报价：${a.commercial_quote || '-'}</div>
            <div class="flex gap-2" style="margin-top:8px;">
              <span class="tag">政治 ${a.risk_political != null ? a.risk_political : '-'}</span>
              <span class="tag">法律 ${a.risk_legal != null ? a.risk_legal : '-'}</span>
              <span class="tag">道德 ${a.risk_moral != null ? a.risk_moral : '-'}</span>
              <span class="tag">商业 ${a.risk_commercial != null ? a.risk_commercial : '-'}</span>
            </div>
          </div>
        </div>
        <div class="card">
          <div class="card-header">🤝 品牌找我 <span class="tag" style="background:#F0F9FF;color:#0369A1;">${inbound.length}</span></div>
          <div class="card-body">${inboundHtml}</div>
        </div>
      </div>
      ...
      <div class="grid-2" style="margin-top:16px;">
        <div class="card">
          <div class="card-header">🏆 我的代言战报 <span class="tag" style="background:#F0F9FF;color:#0369A1;">${endorse.count || 0} 条 / ${endorse.brand_count || 0} 品牌 / GMV ${(endorse.gmv_total_wan || 0)}万</span></div>
          <div class="card-body">
            <div class="table-responsive"><table class="data-table"><thead><tr><th>品牌</th><th>品类</th><th>周期</th><th>GMV</th><th>核对</th></tr></thead><tbody>${endorseHtml}</tbody></table></div>
          </div>
        </div>
        <div class="card">
          <div class="card-header">🔔 被监测动态</div>
  ...
  // ===== 管理方矩阵增强：批量管理 + 同公司风险传导 =====
  _renderMatrixCard(items) {
    const propBadge = (p) => p === '高' ? '<span class="tag tag-risk-high">传导高</span>'
      : p === '中' ? '<span class="tag tag-risk-mid">传导中</span>'
      : '<span class="tag tag-risk-safe">传导低</span>';
    const rows = items.map(x => `
      <tr>
        <td><input type="checkbox" class="mx-chk" value="${x.id}"></td>
        <td><b>${x.name}</b></td>
        <td>${x.heat_level || '-'}</td>
        <td>${x.risk_level || '-'} <span class="text-tertiary text-xs">(${x.risk_score != null ? x.risk_score : '-'})</span></td>
        <td>${x.agency || '无经纪'}</td>
        <td><span class="tag">${x.claim_type_label || x.claim_type}</span></td>
        <td>${x.agency_artist_count} 位 · 均分 ${x.agency_avg_risk != null ? x.agency_avg_risk : '-'}${x.high_risk_peer_count ? ` · <b style="color:var(--risk-high);">${x.high_risk_peer_count}高</b>` : ''}</td>
        <td>${propBadge(x.propagation)}</td>
        <td>
          <button class="btn btn-ghost btn-xs" type="button" onclick="App.navigate('artist/${x.id}')">主页</button>
          ${x.agency ? `<button class="btn btn-ghost btn-xs" type="button" onclick="App.renderMatrixAgencyRisk(${JSON.stringify(x.agency)})">同公司</button>` : ''}
        </td>
      </tr>`).join('');
    return `
      <div class="card" style="margin-top:16px;">
        <div class="card-header">🗂️ 管理方矩阵 · 批量管理 <span class="tag" style="background:#F0F9FF;color:#0369A1;">${items.length} 位</span></div>
        <div class="card-body">
          <div class="flex gap-2" style="margin-bottom:10px;">
            <button class="btn btn-outline btn-sm" type="button" onclick="App._matrixSelectAll()">全选/取消</button>
            <button class="btn btn-primary btn-sm" type="button" onclick="App.exportMatrixCsv()">导出选中 CSV</button>
            <span class="text-tertiary text-xs" style="align-self:center;">勾选艺人批量导出；「同公司」查看同经纪公司风险传导明细</span>
          </div>
          <div class="table-responsive"><table class="data-table" id="matrixTbl">
            <thead><tr><th></th><th>艺人</th><th>热度</th><th>风险</th><th>经纪公司</th><th>管理类型</th><th>同公司风险</th><th>传导</th><th>操作</th></tr></thead>
            <tbody>${rows}</tbody>
          </table></div>
          <div class="text-tertiary text-xs" style="margin-top:8px;">同公司风险传导：依据同经纪公司其他艺人风险分布，给出本艺人连带风险评级（高/中/低）。</div>
        </div>
      </div>`;
  },

  _matrixSelectAll() {
    const all = Array.from(document.querySelectorAll('#matrixTbl .mx-chk'));
  ...
  async exportMatrixCsv() {
    const chk = Array.from(document.querySelectorAll('#matrixTbl .mx-chk')).filter(c => c.checked);
    const ids = chk.map(c => String(c.value));
    if (!ids.length) { this.showToast('请先勾选艺人', 'warn'); return; }
    try {
      const mx = await this.api('/artist/matrix');
      const items = (mx.items || []).filter(x => ids.includes(String(x.id)));
      const header = ['艺人ID','姓名','热度','风险等级','风险分','经纪公司','管理类型','同公司艺人数量','同公司均分','高风险同公司数','传导评级'];
      const lines = items.map(x => [x.id, x.name, x.heat_level || '', x.risk_level || '', x.risk_score != null ? x.risk_score : '', x.agency || '', x.claim_type_label || x.claim_type, x.agency_artist_count || 0, x.agency_avg_risk != null ? x.agency_avg_risk : '', x.high_risk_peer_count || 0, x.propagation || ''].join(','));
      const csv = '﻿' + header.join(',') + '\n' + lines.join('\n');
      const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = '艺安查_管理方矩阵.csv'; a.click();
      URL.revokeObjectURL(url);
      this.showToast(`已导出 ${items.length} 位艺人`, 'success');
    } catch (e) {
      this.showToast('导出失败', 'error');
    }
  },

  async renderMatrixAgencyRisk(agency) {
    try {
      const d = await this.api('/artist/agency-risk/' + encodeURIComponent(agency));
      const dist = d.risk_distribution || {};
      const distHtml = Object.keys(dist).map(k => `<span class="tag">${k}: ${dist[k]}</span>`).join(' ') || '<span class="text-tertiary text-xs">无</span>';
      const high = d.high_risk_artists || [];
      const highHtml = high.length
        ? high.map(h => `<div style="padding:4px 0;">${h.name} <span class="text-tertiary text-xs">（风险分 ${h.risk_score}）</span></div>`).join('')
        : '<div class="text-tertiary text-sm">无高风险艺人</div>';
      const body = `
        <div style="line-height:1.9;">
          <div>艺人总数：<b>${d.total}</b> ｜ 平均风险：<b>${d.avg_risk != null ? d.avg_risk : '-'}</b> ｜ 最高：<b>${d.max_risk != null ? d.max_risk : '-'}</b> ｜ 最低：<b>${d.min_risk != null ? d.min_risk : '-'}</b></div>
          <div style="margin:8px 0;">${distHtml}</div>
          <div style="font-weight:700;margin:10px 0 4px;">⚠️ 高风险艺人（风险分≥70）</div>
          ${highHtml}
          <div class="text-tertiary text-xs" style="margin-top:10px;">同公司风险传导：任一高风险同公司艺人，可能通过品牌联名、捆绑营销对本公司其他艺人产生连带风险。</div>
        </div>`;
      this.openModal('同公司风险传导 · ' + d.agency, body);
    } catch (e) {
  ...
  openModal(title, bodyHtml) {
    let m = document.getElementById('ycModal');
    if (!m) {
      m = document.createElement('div');
      m.id = 'ycModal';
      m.style.cssText = 'position:fixed;inset:0;background:rgba(15,23,42,.5);display:flex;align-items:center;justify-content:center;z-index:9999;padding:20px;';
      m.onclick = (e) => { if (e.target === m) m.remove(); };
      document.body.appendChild(m);
    }
    m.innerHTML = `<div style="background:#fff;border-radius:16px;max-width:560px;width:100%;max-height:82vh;overflow:auto;padding:24px;position:relative;">
      <button type="button" onclick="document.getElementById('ycModal').remove()" style="position:absolute;top:12px;right:14px;border:none;background:none;font-size:22px;line-height:1;cursor:pointer;color:#94A3B8;">×</button>
    ...
    container.innerHTML = `
      ...
      <div class="detail-tabs">
        <div class="detail-tabs-inner">
          ...
          <div class="detail-tab-item" onclick="App.switchTab('graph')">关系图谱</div>
          <div class="detail-tab-item" onclick="App.switchTab('kol')">粉丝力</div>
          <div class="detail-tab-item" onclick="App.switchTab('extra')">扩展画像</div>
          <div class="detail-tab-item" onclick="App.switchTab('family')">家庭标签</div>
        </div>
      </div>
    ...
    try {
      ...
      document.querySelector('.page-content').innerHTML = `
        ...
        <div class="tab-content" id="tab-graph">
          <div id="graphMount"><div class="text-center" style="padding:60px;"><div class="spinner"></div><span style="color:var(--text-tertiary);margin-left:8px;">点击「关系图谱」加载关联网络...</span></div></div>
        </div>

        <div class="tab-content" id="tab-kol">
          <div class="text-center" style="padding:60px;"><div class="spinner"></div><span style="color:var(--text-tertiary);margin-left:8px;">点击「粉丝力」加载 KOL 矩阵...</span></div>
        </div>

        <div class="tab-content" id="tab-extra">
          <div class="text-center" style="padding:60px;"><div class="spinner"></div><span style="color:var(--text-tertiary);margin-left:8px;">点击「扩展画像」加载平台粉丝与画像...</span></div>
        </div>

        <div class="tab-content" id="tab-family">
          <div class="text-center" style="padding:60px;"><div class="spinner"></div><span style="color:var(--text-tertiary);margin-left:8px;">点击「家庭标签」加载家庭/亲子标签...</span></div>
        </div>`;
  ...
  switchTab(tabId) {
    ...
    // 关系图谱 tab：懒加载关联网络（接 /graph/artist/{id}）
    if (tabId === 'graph' && this.state.currentArtistId) {
      this.renderGraphPage(document.getElementById('graphMount'), this.state.currentArtistId);
    }
    // AFF 粉丝力三维度 tab（公开端点 /aff/profile/{id}）
    if (tabId === 'kol' && this.state.currentArtistId) {
      this.renderAffKol(this.state.currentArtistId);
    }
    if (tabId === 'extra' && this.state.currentArtistId) {
      this.renderAffExtra(this.state.currentArtistId);
    }
    if (tabId === 'family' && this.state.currentArtistId) {
      this.renderAffFamily(this.state.currentArtistId);
    }
  },

  // ==================== AFF 粉丝力检索页（独立入口 #aff）====================
  renderAffSearchPage(container) {
    container.innerHTML = `
      <div class="card mb-6">
        <div class="card-body">
          <h2 style="font-size:20px;font-weight:700;margin-bottom:4px;">📡 AFF 粉丝力检索</h2>
          <p style="font-size:13px;color:var(--text-secondary);margin-bottom:14px;">检索 AFF 资料库覆盖的艺人（含选秀 / 潜力小生占位样本），查看其 KOL 矩阵、家庭标签与扩展画像。点击卡片直达艺人详情页。</p>
  ...
  async runAffSearch(name) {
    const input = document.getElementById('affSearchInput');
    const q = (name !== undefined) ? name : (input ? input.value.trim() : '');
    const box = document.getElementById('affResults');
    if (!box) return;
    ...
    try {
          ...
          ${items.map(x => `
            <div class="card" style="padding:14px;cursor:pointer;border:1px solid var(--border,#E5E7EB);" onclick="App.navigate('artist/${x.artist_id}')">
              <div style="display:flex;justify-content:space-between;align-items:center;">
                ...
                ${x.is_placeholder ? '<span class="tag" style="font-size:10px;background:#FEF3C7;color:#92400E;">占位样本</span>' : ''}
              </div>
              <div style="font-size:12px;color:var(--text-tertiary);margin-top:2px;">${x.heat_level || '-'} · ${x.agency || '无经纪'}</div>
      ...
      box.innerHTML = `<div style="color:var(--text-tertiary);font-size:12px;">检索失败</div>`;
    }
  },

  // ==================== 潜力 Seeding 艺人榜（独立入口 #seeding）====================
  renderSeedingPage(container) {
    container.innerHTML = `
      <div class="card mb-6">
        <div class="card-body">
          <h2 style="font-size:20px;font-weight:700;margin-bottom:4px;">🌱 潜力 Seeding 艺人榜</h2>
          <p style="font-size:13px;color:var(--text-secondary);margin-bottom:12px;">AFF 内部推荐的上升艺人（共 51 位），含类型与推荐理由。点击卡片查看其粉丝力健康度与核心粉丝站明细。</p>
          <div class="search-box" style="max-width:480px;">
            <input type="text" id="seedSearch" placeholder="搜索艺人，如 田栩宁 / 梓渝" onkeydown="if(event.key==='Enter')App.runSeedingSearch()">
            <button class="search-btn" onclick="App.runSeedingSearch()">搜索</button>
          </div>
          <div id="seedFilters" style="margin-top:10px;display:flex;gap:6px;flex-wrap:wrap;"></div>
        </div>
      </div>
      <div id="seedResults"></div>`;
    this.runSeedingSearch();
  },

  async runSeedingSearch(q, category) {
    q = (q !== undefined) ? q : (document.getElementById('seedSearch') ? document.getElementById('seedSearch').value.trim() : '');
    const box = document.getElementById('seedResults');
    if (!box) return;
    box.innerHTML = `<div style="text-align:center;padding:20px;"><div class="spinner"></div></div>`;
    try {
      let url = '/aff/seeding?limit=100';
      if (q) url += '&q=' + encodeURIComponent(q);
      if (category) url += '&category=' + encodeURIComponent(category);
      const d = await this.api(url);
      const fbox = document.getElementById('seedFilters');
      if (fbox && !q) {
        const cats = (d.category_distribution || []).filter(c => c.count > 0);
        fbox.innerHTML = `<span style="font-size:12px;color:var(--text-tertiary);margin-right:4px;">类型：</span>` +
          `<button class="tag" style="cursor:pointer;border:none;background:#0369A1;color:#fff;" onclick="App.runSeedingSearch('','')">全部</button> ` +
          cats.map(c => `<button class="tag" style="cursor:pointer;border:none;background:#EEF2FF;color:#4338CA;" onclick="App.runSeedingSearch('','${this.esc(c.category)}')">${this.esc(c.category)} ${c.count}</button>`).join(' ');
      }
      const items = d.items || [];
      if (!items.length) { box.innerHTML = '<div style="text-align:center;color:var(--text-tertiary);padding:30px;">无匹配艺人</div>'; return; }
      box.innerHTML = `<div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px;">共 ${d.total} 位</div>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:12px;">
        ${items.map(x => `<div class="card" style="padding:14px;cursor:pointer;border:1px solid var(--border,#E5E7EB);" onclick="App.showKolDetailModal(${x.artist_id})">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <div style="font-weight:700;font-size:15px;">${this.esc(x.name)}</div>
            ${x.has_kol ? '<span class="tag" style="font-size:10px;background:#ECFDF5;color:#047857;">有站子数据</span>' : '<span class="tag" style="font-size:10px;background:#F3F4F6;color:#6B7280;">待补</span>'}
          </div>
          <div style="margin-top:4px;">
            ${x.category ? `<span class="tag" style="font-size:10px;background:#EEF2FF;color:#4338CA;">${this.esc(x.category)}</span>` : '<span class="text-tertiary text-xs">未分类</span>'}
            ${x.gender ? `<span class="text-tertiary text-xs"> · ${x.gender}</span>` : ''}
            ${x.kol_count ? `<span class="text-tertiary text-xs"> · ${x.kol_count} 站</span>` : ''}
          </div>
          ${x.reason ? `<div style="margin-top:8px;font-size:12px;color:var(--text-secondary);line-height:1.5;">${this.esc(x.reason)}</div>` : ''}
        </div>`).join('')}
        </div>`;
    } catch (e) {
  ...
  async _loadAff(id) {
    if (this.state.affId === id && this.state.affProfile) return this.state.affProfile;
    try {
      const d = await this.api('/aff/profile/' + id);
      this.state.affId = id;
      this.state.affProfile = d;
      return d;
    } catch (e) {
      return null;
    }
  ...
  _affEmpty(label) {
    return `<div style="text-align:center;padding:50px;color:var(--text-tertiary);font-size:14px;">
      📭 该艺人暂无「${label}」数据（AFF 粉丝力资料库尚未覆盖）</div>`;
  },

  async renderAffKol(id) {
    ...
    if (det) {
      const h = det.health || {};
      const tierColor = h.tier === '强' ? '#059669' : h.tier === '中' ? '#D97706' : '#0369A1';
      const maxFans = Math.max(1, ...det.platform_stats.map(p => p.fans));
      const bars = det.platform_stats.map(p => `
        <div style="display:flex;align-items:center;gap:8px;margin:6px 0;font-size:12px;">
          <div style="width:52px;color:var(--text-secondary);">${this.esc(p.platform)}</div>
          <div style="flex:1;background:var(--gray-100,#F3F4F6);border-radius:6px;overflow:hidden;height:16px;">
            <div style="width:${(p.fans / maxFans * 100).toFixed(1)}%;background:#0369A1;height:100%;"></div>
          </div>
          <div style="width:96px;text-align:right;color:var(--text-secondary);">${this.fmtInt(p.fans)} · ${p.count}站</div>
        </div>`).join('');
      const top = (det.top10 || []).map((x, i) => `<tr>
        <td style="width:28px;color:var(--text-tertiary);">${i + 1}</td>
        <td>${x.link ? `<a href="${this.esc(x.link)}" target="_blank" rel="noopener" style="color:#0369A1;">${this.esc(x.station_name)}</a>` : this.esc(x.station_name)}</td>
        <td><span class="tag" style="font-size:10px;background:#EEF2FF;color:#4338CA;">${this.esc(x.platform)}</span></td>
      ...
      <div class="card" style="margin-bottom:16px;">
        ...
        <div class="card-body">
          <div style="display:flex;gap:24px;flex-wrap:wrap;align-items:center;">
            <div style="text-align:center;">
              <div style="font-size:40px;font-weight:800;color:${tierColor};line-height:1;">${h.score != null ? h.score : '-'}</div>
              <div style="font-size:12px;color:${tierColor};font-weight:700;margin-top:2px;">${h.tier || ''} 梯队</div>
            </div>
            <div style="display:flex;gap:22px;flex-wrap:wrap;">
              <div><div style="font-size:22px;font-weight:800;">${this.fmtInt(det.total_fans)}</div><div class="text-tertiary text-xs">总粉丝量级</div></div>
              <div><div style="font-size:22px;font-weight:800;">${det.total_stations}</div><div class="text-tertiary text-xs">核心站子数</div></div>
              <div><div style="font-size:22px;font-weight:800;">${det.platform_stats.length}</div><div class="text-tertiary text-xs">覆盖平台</div></div>
            </div>
          </div>
      ...
      <div class="card" style="margin-bottom:16px;">
        ...
        <div class="card-body"><div class="table-responsive"><table class="data-table">
          <thead><tr><th>#</th><th>站子/粉丝KOL</th><th>平台</th><th style="text-align:right;">粉丝数</th><th>分类</th></tr></thead>
          <tbody>${top}</tbody></table></div></div>
      </div>`;
    }
    // 旧聚合矩阵（若存在）
    if (d && d.kol_matrix) {
      const k = d.kol_matrix;
      const bp = k.by_platform || {};
      const platChips = Object.entries(bp).map(([p, c]) => `<span class="tag" style="background:#EEF2FF;color:#4338CA;">${p} ${this.fmtInt(c)}</span>`).join(' ');
      const topk = (k.top_kols || []).slice(0, 15).map(x => `<tr>
        <td>${x['平台'] || '-'}</td>
        <td>${x['链接'] ? `<a href="${this.esc(x['链接'])}" target="_blank" rel="noopener" style="color:#4F46E5;">${this.esc(x['昵称'] || '')}</a>` : (this.esc(x['昵称'] || '-'))}</td>
        <td style="text-align:right;">${this.fmtInt(x['粉丝数'])}</td>
        <td>${this.esc(x['分类'] || '-')}</td>
      </tr>`).join('');
      html += `
      <div class="card" style="margin-bottom:16px;">
        <div class="card-header">📡 KOL / 粉丝站矩阵（聚合）<span class="tag" style="background:#F0F9FF;color:#0369A1;">AFF 粉丝力</span>${k.is_reference ? '<span class="tag" style="background:#FEF3C7;color:#92400E;margin-left:6px;">参考样本</span>' : ''}</div>
        <div class="card-body">
          <div style="display:flex;gap:28px;flex-wrap:wrap;align-items:flex-end;">
            <div><div style="font-size:26px;font-weight:800;color:#4F46E5;">${this.fmtInt(k.kol_total)}</div><div class="text-tertiary text-xs">KOL / 粉丝站总数</div></div>
            <div><div style="font-size:26px;font-weight:800;">${this.fmtInt(k.fan_quality)}</div><div class="text-tertiary text-xs">粉丝质量 / 总粉丝量级</div></div>
          </div>
          <div style="margin-top:12px;">${platChips || '<span class="text-tertiary text-xs">无平台分布数据</span>'}</div>
        </div>
      </div>
      <div class="card">
        <div class="card-header">🔝 Top KOL / 粉丝站（按粉丝数）</div>
        <div class="card-body"><div class="table-responsive"><table class="data-table">
          <thead><tr><th>平台</th><th>昵称</th><th style="text-align:right;">粉丝数</th><th>分类</th></tr></thead>
          <tbody>${topk || '<tr><td colspan="4" style="text-align:center;color:var(--text-tertiary);">暂无明细</td></tr>'}</tbody>
        </table></div></div>
      </div>`;
    }
    if (!html) {
      el.innerHTML = this._affEmpty('粉丝力 / KOL 矩阵');
    } else {
      el.innerHTML = html;
    }
  },

  // 核心粉丝站明细弹窗（P0）：复用 /aff/kol-details 全量站子
  async showKolDetailModal(id) {
    let m = document.getElementById('kolModal');
    if (!m) {
      m = document.createElement('div');
      m.id = 'kolModal';
      m.style.cssText = 'position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:9999;display:none;align-items:flex-start;justify-content:center;overflow:auto;padding:40px 16px;';
      document.body.appendChild(m);
      m.onclick = (e) => { if (e.target === m) m.style.display = 'none'; };
    }
    m.innerHTML = `<div style="background:var(--bg,#fff);max-width:780px;width:100%;border-radius:14px;box-shadow:0 20px 60px rgba(0,0,0,.3);"><div id="kolModalBody" style="padding:24px;"><div class="text-center" style="padding:50px;"><div class="spinner"></div></div></div></div>`;
    m.style.display = 'flex';
    try {
      const det = await this.api('/aff/kol-details/' + id);
      const maxFans = Math.max(1, ...det.platform_stats.map(p => p.fans));
      const bars = det.platform_stats.map(p => `<div style="display:flex;align-items:center;gap:8px;margin:6px 0;font-size:12px;">
        <div style="width:52px;color:var(--text-secondary);">${this.esc(p.platform)}</div>
        <div style="flex:1;background:var(--gray-100,#F3F4F6);border-radius:6px;height:16px;overflow:hidden;"><div style="width:${(p.fans / maxFans * 100).toFixed(1)}%;background:#0369A1;height:100%;"></div></div>
        <div style="width:96px;text-align:right;color:var(--text-secondary);">${this.fmtInt(p.fans)} · ${p.count}站</div></div>`).join('');
      const rows = (det.stations || []).map((x, i) => `<tr>
        <td style="width:28px;color:var(--text-tertiary);">${i + 1}</td>
        <td>${x.link ? `<a href="${this.esc(x.link)}" target="_blank" rel="noopener" style="color:#0369A1;">${this.esc(x.station_name)}</a>` : this.esc(x.station_name)}</td>
        <td><span class="tag" style="font-size:10px;background:#EEF2FF;color:#4338CA;">${this.esc(x.platform)}</span></td>
        <td style="text-align:right;">${this.fmtInt(x.fans)}</td>
        <td style="color:var(--text-secondary);">${this.esc(x.category || '-')}</td></tr>`).join('');
      const h = det.health || {};
      const tColor = h.tier === '强' ? '#059669' : h.tier === '中' ? '#D97706' : '#0369A1';
      document.getElementById('kolModalBody').innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
          <div style="font-size:18px;font-weight:800;">🔥 ${this.esc(det.artist_name)} · 全部核心粉丝站（${det.total_stations}）</div>
          <button class="btn btn-ghost btn-sm" onclick="document.getElementById('kolModal').style.display='none'">✕</button>
        </div>
        <div style="display:flex;gap:22px;flex-wrap:wrap;margin-bottom:12px;">
          <div><div style="font-size:22px;font-weight:800;color:#0369A1;">${this.fmtInt(det.total_fans)}</div><div class="text-tertiary text-xs">总粉丝量级</div></div>
          <div><div style="font-size:22px;font-weight:800;">${h.score != null ? h.score : '-'} <span style="font-size:12px;color:${tColor};">${h.tier || ''}</span></div><div class="text-tertiary text-xs">粉丝力健康度</div></div>
          <div><div style="font-size:22px;font-weight:800;">${det.total_stations}</div><div class="text-tertiary text-xs">核心站子数</div></div>
        </div>
        <div style="margin-bottom:10px;font-size:13px;font-weight:600;color:var(--text-secondary);">平台粉丝分布</div>
        ${bars}
        <div class="table-responsive" style="margin-top:14px;"><table class="data-table" style="font-size:12px;">
          <thead><tr><th>#</th><th>站子/粉丝KOL</th><th>平台</th><th style="text-align:right;">粉丝数</th><th>分类</th></tr></thead>
          <tbody>${rows}</tbody></table></div>`;
    } catch (e) {
      document.getElementById('kolModalBody').innerHTML = '<div style="padding:30px;text-align:center;color:var(--text-tertiary);">加载失败</div>';
    }
  },

  async renderAffExtra(id) {
    const el = document.getElementById('tab-extra');
    if (!el) return;
    const d = await this._loadAff(id);
    if (!d || !d.extra_profile) { el.innerHTML = this._affEmpty('扩展画像'); return; }
    const p = d.extra_profile;
    const stat = (label, val) => `<div style="flex:1;min-width:120px;"><div style="font-size:20px;font-weight:800;">${val === '' || val === null || val === undefined ? '-' : val}</div><div class="text-tertiary text-xs">${label}</div></div>`;
    const brands = (p['合作品牌'] || []).map(b => `<span class="tag" style="background:#ECFDF5;color:#047857;">${b}</span>`).join(' ');
    const works = (p['代表作品'] || []).filter(w => (w['剧名'] || '').trim()).map(w => `<tr>
      <td>${w['剧名'] || '-'}</td><td>${w['饰演'] || '-'}</td><td>${w['合作演员'] || '-'}</td><td>${w['平台'] || '-'}</td>
    </tr>`).join('');
    el.innerHTML = `
      <div class="card" style="margin-bottom:16px;">
        <div class="card-header">📊 平台粉丝与基础画像 <span class="tag" style="background:#F0F9FF;color:#0369A1;">AFF 扩展画像</span></div>
        <div class="card-body">
          <div style="display:flex;gap:18px;flex-wrap:wrap;">
            ${stat('抖音粉丝(万)', p['抖音粉丝(万)'])}
            ${stat('微博粉丝(万)', p['微博粉丝(万)'])}
            ${stat('小红书粉丝(万)', p['小红书粉丝(万)'])}
            ${stat('获赞(亿)', p['获赞(亿)'])}
            ${stat('播放量(亿)', p['播放量(亿)'])}
            ${stat('近30天热搜', p['近30天热搜'])}
            ${stat('日活粉丝', p['日活粉丝'])}
            ${stat('百度指数', p['百度指数'])}
            ${stat('女性占比', p['女性占比'])}
            ${stat('主力年龄', p['主力年龄'])}
          </div>
          <div style="margin-top:14px;font-size:13px;"><b>经纪公司：</b>${p['经纪公司'] || '-'}</div>
          <div style="margin-top:8px;font-size:13px;"><b>选秀出身：</b>${p['选秀出身'] || '非选秀'}</div>
          <div style="margin-top:8px;font-size:13px;"><b>粉丝特征：</b>${p['粉丝特征'] || '-'}</div>
        </div>
      </div>
      ${brands ? `<div class="card" style="margin-bottom:16px;"><div class="card-header">🤝 合作品牌</div><div class="card-body" style="line-height:2.2;">${brands}</div></div>` : ''}
      ${works ? `<div class="card"><div class="card-header">🎬 代表作品</div><div class="card-body"><div class="table-responsive"><table class="data-table">
        <thead><tr><th>剧名</th><th>饰演</th><th>合作演员</th><th>平台</th></tr></thead>
        <tbody>${works}</tbody></table></div></div>` : ''}`;
  },

  async renderAffFamily(id) {
    const el = document.getElementById('tab-family');
    if (!el) return;
    const d = await this._loadAff(id);
    if (!d || !d.family_tags) { el.innerHTML = this._affEmpty('家庭 / 亲子标签'); return; }
    const f = d.family_tags;
    el.innerHTML = `
      <div class="card" style="margin-bottom:16px;">
        <div class="card-header">💞 家庭 / 亲子标签 <span class="tag" style="background:#FDF2F8;color:#BE185D;">AFF 人设叙事</span></div>
        <div class="card-body" style="display:flex;flex-direction:column;gap:14px;">
          ${f.marriage_status ? `<div><div class="text-tertiary text-xs">夫妻关系</div><div style="font-size:14px;margin-top:4px;">${f.marriage_status}</div></div>` : ''}
          ${f.children ? `<div><div class="text-tertiary text-xs">子女</div><div style="font-size:14px;margin-top:4px;">${f.children}</div></div>` : ''}
          ${f.persona ? `<div><div class="text-tertiary text-xs">人设 / 适配叙事</div><div style="font-size:14px;margin-top:4px;line-height:1.7;">${f.persona}</div></div>` : ''}
          ${f.works_highlight ? `<div><div class="text-tertiary text-xs">作品 / 综艺 / 出圈点</div><div style="font-size:14px;margin-top:4px;">${f.works_highlight}</div></div>` : ''}
        </div>
      </div>
      <div class="text-tertiary text-xs" style="margin-top:8px;">⚠️ 家庭/亲子标签用于品牌「母婴、亲子出行、女性成长」等正向叙事匹配，不涉及隐私挖掘，输出为「评估参考」非「事实保证」。</div>`;
  },

  async renderGraphPage(container, id) {
    if (!id) return;
    ...
    container.innerHTML = `<div class="page-content" style="max-width:1100px;margin:0 auto;padding:var(--space-6);">
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex gap-2">
          <select id="graphDepth" class="form-input" style="max-width:180px;">
            <option value="shallow">浅层（同公司+对家）</option>
            <option value="deep">深层（二级关联）</option>
          </select>
          <button class="btn btn-outline btn-sm" type="button" onclick="App._reloadGraph(${id})">刷新</button>
        </div>
      </div>
      <div id="graphCanvas" style="height:520px;background:var(--bg,#fff);border:1px solid var(--border-light,#E2E8F0);border-radius:16px;"></div>
      <div id="graphLegend" class="text-sm text-secondary" style="margin-top:12px;"></div>
    </div>`;
    await this._reloadGraph(id);
  },

  async _reloadGraph(id) {
    const canvas = document.getElementById('graphCanvas');
    if (!canvas) return;
    const depthSel = document.getElementById('graphDepth');
    const depth = (depthSel && depthSel.value) || 'shallow';
    canvas.innerHTML = '<div class="loading-overlay"><div class="spinner"></div></div>';
    try {
      const data = await this.api('/graph/artist/' + id + '?depth=' + depth);
      await this._ensureVisNetwork();
      const relLabel = { belongs_to: '隶属', same_agency: '同公司', potential_rival: '潜在对家', fan_rival: '粉丝对家', shared_brand: '同品牌' };
      const nodes = new vis.DataSet((data.nodes || []).map(n => ({
        id: n.id,
        label: n.name,
        group: n.node_type,
        title: (n.heat_level || '') + ' / ' + (n.risk_level || ''),
        color: this._graphNodeColor(n),
        size: n.level === 2 ? 9 : (n.level === 1 ? 13 : 18),
      })));
      const edges = new vis.DataSet((data.edges || []).map(e => ({
        from: e.source, to: e.target,
        label: relLabel[e.relation_type] || e.relation_type,
      ...
      new vis.Network(el, { nodes, edges }, {
        ...
        edges: { arrows: 'to', smooth: { type: 'cubicBezier', roundness: 0.3 } },
        groups: {
          artist: { color: { background: '#6366F1', border: '#4338CA' } },
          agency: { color: { background: '#F59E0B', border: '#D97706' }, shape: 'square' },
          rival_group: { color: { background: '#EF4444', border: '#DC2626' } },
        },
      });
      const legend = document.getElementById('graphLegend');
      if (legend) legend.innerHTML = '共 ' + (data.total_nodes || (data.nodes || []).length) + ' 个节点 / ' + (data.total_edges || (data.edges || []).length) + ' 条关系 ｜ 深度：' + (data.depth === 'deep' ? '二级关联' : '浅层') + ' ｜ 大=中心，中=一级关联，小=二级关联（同品牌）';
    } catch (e) {
      canvas.innerHTML = '<div class="text-secondary text-sm">图谱加载失败：' + ((e && e.message) || '未知错误') + '</div>';
    }
  },
  ...
  _graphNodeColor(n) {
    if (n.level === 2) {
      if (n.node_type === 'agency') return { background: '#FCD34D', border: '#D97706' };
      const rl = n.risk_level;
      if (rl === '高风险') return { background: '#FCA5A5', border: '#EF4444' };
      if (rl === '中风险' || rl === '中高风险') return { background: '#FCD34D', border: '#F59E0B' };
      return { background: '#C7D2FE', border: '#6366F1' };
    }
    if (n.node_type === 'agency') return { background: '#F59E0B', border: '#D97706' };
    if (n.node_type === 'rival_group') return { background: '#EF4444', border: '#DC2626' };
    const rl = n.risk_level;
    if (rl === '高风险') return { background: '#EF4444', border: '#DC2626' };
    if (rl === '中风险' || rl === '中高风险') return { background: '#F59E0B', border: '#D97706' };
    return { background: '#6366F1', border: '#4338CA' };
  },
  ...
  renderSalesReports(box, s, list) {
    ...
    const leaderboardBtn = `<div style="margin-top:14px;text-align:center;">
      <div style="display:inline-flex;gap:8px;flex-wrap:wrap;justify-content:center;margin-bottom:6px;">
        <button class="btn btn-ghost" style="font-size:12px;padding:6px 14px;" onclick="App.toggleSalesLeaderboard()">📊 全行业代言战报榜 TOP20</button>
        <button class="btn btn-ghost" style="font-size:12px;padding:6px 14px;color:#0369A1;border-color:#0369A1;" onclick="App.toggleAffOverview()">📡 全行业 AFF 粉丝力榜</button>
      </div>
  ...
  async toggleSalesLeaderboard() {
    ...
    if (!box) return;
    if (box.style.display !== 'none' && box.dataset.mode === 'sales') { box.style.display = 'none'; box.innerHTML = ''; box.dataset.mode = ''; return; }
    box.dataset.mode = 'sales';
  ...
  async toggleAffOverview() {
    const box = document.getElementById('salesLeaderboardBox');
    if (!box) return;
    // 与代言战报榜互斥：再次点击收起
    if (box.style.display !== 'none' && box.dataset.mode === 'aff') { box.style.display = 'none'; box.innerHTML = ''; box.dataset.mode = ''; return; }
    box.dataset.mode = 'aff';
    box.style.display = 'block';
    box.innerHTML = `<div style="text-align:center;padding:14px;"><div class="spinner"></div></div>`;
    try {
      const d = await this.api('/aff/overview');
      const cov = d.coverage || {};
      const topKol = (d.top_kol || []).map((k, i) => `<tr>
        <td style="width:28px;color:var(--text-tertiary);">${i + 1}</td>
        <td style="font-weight:600;">${k.name}</td>
        <td>${k.kol_total}</td>
        <td style="color:var(--text-tertiary);font-size:11px;">微博 ${(k.by_platform && k.by_platform['微博']) || 0} · 小红书 ${(k.by_platform && k.by_platform['小红书']) || 0}</td>
      </tr>`).join('');
      const fd = d.family_distribution || {};
      const pc = d.platform_coverage || {};
      box.innerHTML = `<div style="font-size:12px;color:var(--text-secondary);margin-bottom:6px;">AFF 粉丝力资料库覆盖：KOL矩阵 <b>${cov.kol_matrix || 0}</b> 人 · 家庭标签 <b>${cov.family_tags || 0}</b> 人 · 扩展画像 <b>${cov.extra_profile || 0}</b> 人</div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:10px;">
          <div class="card" style="padding:10px;"><div style="font-size:11px;color:var(--text-tertiary);">家庭标签分布</div><div style="font-size:12px;margin-top:4px;">有婚姻 ${fd['有婚姻记录'] || 0} · 有子女 ${fd['有子女记录'] || 0}</div></div>
          <div class="card" style="padding:10px;"><div style="font-size:11px;color:var(--text-tertiary);">平台画像覆盖</div><div style="font-size:12px;margin-top:4px;">抖音 ${pc['抖音粉丝'] || 0} · 小红书 ${pc['小红书粉丝'] || 0} · 微博 ${pc['微博粉丝'] || 0}</div></div>
          <div class="card" style="padding:10px;"><div style="font-size:11px;color:var(--text-tertiary);">指数 / 画像</div><div style="font-size:12px;margin-top:4px;">百度 ${pc['百度指数'] || 0} · 女性占比 ${pc['女性占比'] || 0}</div></div>
        </div>
        <div style="font-size:12px;color:var(--text-secondary);margin-bottom:4px;">KOL / 粉丝站矩阵 TOP12（按粉丝站总数）</div>
        <div class="table-responsive"><table class="data-table"><thead><tr><td>#</td><td>艺人</td><td>粉丝站总数</td><td>平台分布</td></tr></thead><tbody>${topKol}</tbody></table></div>
        <div style="font-size:11px;color:var(--text-tertiary);margin-top:6px;">数据来源：AFF 粉丝力分析资料库（粉丝站统计 / 家庭标签 / 平台画像），仅供品牌评估参考，非事实保证。</div>`;
    } catch (e) {
      box.innerHTML = `<div style="color:var(--text-tertiary);font-size:12px;">AFF 看板加载失败</div>`;
    }
  },
