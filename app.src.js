...
const App = {
  ...
  async renderAffKol(id) {
    ...
    if (!d || !d.kol_matrix) { el.innerHTML = this._affEmpty('粉丝力 / KOL 矩阵'); return; }
    ...
    const top = (k.top_kols || []).slice(0, 15).map(x => `<tr>
      <td>${x['平台'] || '-'}</td>
      <td>${x['链接'] ? `<a href="${x['链接']}" target="_blank" rel="noopener" style="color:var(--accent-indigo,#4F46E5);">${x['昵称'] || ''}</a>` : (x['昵称'] || '-')}</td>
      <td style="text-align:right;">${this.fmtInt(x['粉丝数'])}</td>
      <td>${x['分类'] || '-'}</td>
    </tr>`).join('');
    el.innerHTML = `
      <div class="card" style="margin-bottom:16px;">
        <div class="card-header">📡 KOL / 粉丝站矩阵 <span class="tag" style="background:#F0F9FF;color:#0369A1;">AFF 粉丝力</span>${k.is_reference ? '<span class="tag" style="background:#FEF3C7;color:#92400E;margin-left:6px;">参考样本</span>' : ''}</div>
        <div class="card-body">
          <div style="display:flex;gap:28px;flex-wrap:wrap;align-items:flex-end;">
            <div><div style="font-size:26px;font-weight:800;color:var(--accent-indigo,#4F46E5);">${this.fmtInt(k.kol_total)}</div><div class="text-tertiary text-xs">KOL / 粉丝站总数</div></div>
      ...
      <div class="card">
        ...
        <div class="card-body"><div class="table-responsive"><table class="data-table">
          ...
          <tbody>${top || '<tr><td colspan="4" style="text-align:center;color:var(--text-tertiary);">暂无明细</td></tr>'}</tbody>
        </table></div></div>
      </div>`;
  },
