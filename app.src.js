...
const App = {
  ...
  async _reloadGraph(id) {
    ...
    try {
      ...
      if (legend) legend.innerHTML = '共 ' + (data.total_nodes || (data.nodes || []).length) + ' 个节点 / ' + (data.total_edges || (data.edges || []).length) + ' 条关系（中心艺人高亮）';
    } catch (e) {
