...
(function initChartDefaults() {
  ...
  Chart.defaults.borderColor = 'rgba(130, 150, 220, 0.16)';

  // 元素级精致化：平滑曲线 / 圆角柱 / 隐藏常态点（hover 显形）
  try {
    if (Chart.defaults.elements) {
      ...
      if (Chart.defaults.elements.arc) {
        ...
        Chart.defaults.elements.arc.hoverOffset = 10;
      }
    }
  } catch (e) { console.warn('Chart element defaults failed:', e); }

  console.log('✅ Chart.js 全局配置已优化');
})();

const App = {API_BASE: 'https://yiancha-backend.onrender.com/api/v1',state: {
    user: null,
    ...
    // 视觉升级：明暗主题 + 粒子背景 + 更多菜单外部点击关闭
    this.applyTheme();
    if (document.documentElement.getAttribute('data-theme') !== 'light') this.initBackground();
    ...
    this.checkForUpdate();
  },_updateTimer: null,checkForUpdate() {
    ...
    const saved = localStorage.getItem('yc_theme');
    ...
    if (next === 'dark') this.initBackground(); else this.stopBackground();
  },toggleMore(e) {
    if (e) e.stopPropagation();
    const el = document.getElementById('navMore');
    if (el) el.classList.toggle('open');
  },_bgRunning: false,_bgRAF: null,initBackground() {
    ...
    let w, h, dpr, pts;
    const palette = ['#7C8CFF', '#22D3EE', '#A78BFA', '#F472B6'];
    const resize = () => {
      dpr = Math.min(window.devicePixelRatio || 1, 2);
      w = cv.width = innerWidth * dpr; h = cv.height = innerHeight * dpr;
      cv.style.width = innerWidth + 'px'; cv.style.height = innerHeight + 'px';
      const n = Math.min(72, Math.floor(innerWidth * innerHeight / 26000));
      pts = Array.from({ length: n }, () => ({
        x: Math.random() * w, y: Math.random() * h,
        vx: (Math.random() - 0.5) * 0.25 * dpr, vy: (Math.random() - 0.5) * 0.25 * dpr,
        r: (Math.random() * 1.6 + 0.6) * dpr, c: palette[(Math.random() * palette.length) | 0]
      }));
    };
    resize();
    ...
    const draw = () => {
      ...
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (Math.hypot(dx, dy) < 130 * dpr) {
            ctx.strokeStyle = a.c; ctx.lineWidth = dpr * 0.6;
    ...
    } else if (hash === 'billing' || hash === 'pricing') {
      this.renderBillingPage(container);
    } else {
        ...
        <div class="card-body"><div style="display:flex;flex-wrap:wrap;gap:12px;">
          ${mArt.map(x => `
            <div style="border:1px solid var(--border-light);border-radius:12px;padding:12px;min-width:180px;">
              ...
              <div class="text-tertiary text-xs">${x.heat_level || '-'} · ${x.risk_level || '-'} · ${this.maskedInline(x.agency, '经纪公司')}</div>
    ...
    const rows = items.map(x => `
      <tr>
        ...
        <td>${this.maskedInline(x.agency, '经纪公司')}</td>
  ...
  },openAuth() { document.getElementById('authModal').classList.add('show'); },closeAuth() {
  ...
  },openBusinessModal() { document.getElementById('businessModal').classList.add('show'); },closeBusinessModal() { document.getElementById('businessModal').classList.remove('show'); },onPricingSideChange() {
  ...
  },closePricing() { const m = document.getElementById('pricingModal'); if (m) m.classList.remove('show'); },async submitPricing() {
    const side = document.getElementById('pzSide').value;
  ...
  },closeVerify() { const m = document.getElementById('verifyModal'); if (m) m.classList.remove('show'); },async submitVerify() {
  ...
  },openIntake() { document.getElementById('intakeModal').classList.add('show'); },closeIntake() { const m = document.getElementById('intakeModal'); if (m) m.classList.remove('show'); },async submitIntake(e) {
  ...
  },maskedInline(v, label) {
    return this.isMasked(v)
      ? `<span class="tier-masked" title="${this.escHtml(label || '该信息')}"><span class="tier-lock">🔒</span>登录后可见</span>`
      : (v || '-');
  ...
  },async checkoutPlan(plan, cycle = 'monthly', channel) {
    const names = { personal: 'C 端会员', professional: '专业版', enterprise: '企业版' };
    const ch = channel || this.state.payChannel || 'sandbox';
    const m = document.getElementById('upgradeModal');
    if (m) m.classList.remove('show');
    try {
      const res = await this.api('/billing/checkout', {
        method: 'POST',
        body: JSON.stringify({ plan_type: plan, billing_cycle: cycle, channel: ch }),
      });
      if (res.pay_status === 'paid' && res.auto_activated) {
        this.showToast(`已开通「${names[plan] || plan}」，权益立即生效`, 'success');
        this.state.plan = plan; this.route();
        return;
      }
      // 沙箱支付：弹出二维码 + 模拟支付按钮（真实接入后由微信/支付宝回调自动激活）
      this.openPayModal(res, names[plan] || plan);
    } catch (e) {
      this.showToast('下单失败：' + (e.message || '请重试'), 'error');
    }
  },async checkoutSingleReport() {
    const m = document.getElementById('upgradeModal');
    if (m) m.classList.remove('show');
    this.showToast('单份深度报告已并入「专业版」权益，请直接升级专业版', 'info');
    setTimeout(() => this.openUpgradeModal('whitelabel'), 600);
  },openPayModal(res, planName) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay show';
    overlay.id = 'payModal';
    const method = res.pay_method || 'sandbox';
    const qr = this._qrDataUrl(res.pay_qr || res.pay_url || '');
    let mediaHtml = '';
    if (method === 'wechat' && res.pay_qr) {
      mediaHtml = `<div class="pay-qr"><img src="${qr}" alt="wechat qr" /></div>
        <div style="font-size:12px;color:var(--text-tertiary);">请使用微信「扫一扫」完成支付</div>`;
    } else if (method === 'alipay' && res.pay_url) {
      mediaHtml = `<a class="btn btn-primary" style="width:100%;justify-content:center;padding:14px;font-size:15px;margin-bottom:8px"
          href="${res.pay_url}" target="_blank">🔵 点击跳转支付宝支付</a>
        <div style="font-size:12px;color:var(--text-tertiary);">跳转后完成付款，系统自动激活</div>`;
    } else {
      mediaHtml = `<div class="pay-qr"><img src="${qr}" alt="pay qr" /></div>
        <div style="font-size:12px;color:var(--text-tertiary);margin-bottom:6px">沙箱演示支付（无真实扣款）· 生产将接微信/支付宝</div>`;
    }
    overlay.innerHTML = `
      <div class="modal-card pay-card">
        <div class="modal-close" onclick="document.getElementById('payModal').remove()">×</div>
        <h3 style="margin-bottom:4px">${planName} · 支付</h3>
        ${mediaHtml}
        <div class="pay-amount">¥${res.pay_amount} <small>元</small></div>
        <div class="pay-meta">订单号 ${res.order_no}</div>
        ${(method === 'sandbox') ? `<button class="btn btn-primary" style="width:100%;justify-content:center;margin-top:10px"
          onclick="App.confirmPay('${res.order_no}')">我已支付（模拟回调激活）</button>` : ''}
      </div>`;
    document.body.appendChild(overlay);
  },async confirmPay(orderNo) {
    try {
      const res = await this.api('/billing/notify', { method: 'POST', body: JSON.stringify({ order_no: orderNo, sandbox: true }) });
      const pm = document.getElementById('payModal'); if (pm) pm.remove();
      if (res.activated) {
        this.showToast(`支付成功，已开通${res.user_type === 'enterprise' ? '企业版' : '专业版'}，权益立即生效`, 'success');
        this.state.plan = res.user_type; this.route();
      }
    } catch (e) {
      this.showToast('激活失败：' + (e.message || '请重试'), 'error');
    }
  },_qrDataUrl(text) {
    try {
    ...
    const svg = encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160"><rect width="160" height="160" fill="#fff"/><text x="80" y="84" font-size="11" text-anchor="middle" fill="#161a2e" font-family="monospace">扫码支付</text><text x="80" y="104" font-size="9" text-anchor="middle" fill="#888">${text.slice(0, 28)}</text></svg>`);
    ...
    const meta = [this.maskedInline(a.agency, '经纪公司'), a.gender === '男' ? '男' : a.gender === '女' ? '女' : '', a.age ? a.age + '岁' : ''].filter(Boolean).join(' · ') || '-';
    const tags = [];
    ...
    try {
      ...
      container.innerHTML = `
        <div class="stat-card" data-accent="primary"><div class="stat-number">${total}</div><div class="stat-label">收录艺人</div></div>
        <div class="stat-card" data-accent="safe"><div class="stat-number" style="color:var(--risk-safe);">${lowRisk}</div><div class="stat-label">低风险艺人</div></div>
        <div class="stat-card" data-accent="warn"><div class="stat-number" style="color:var(--heat-s);">${sCount}</div><div class="stat-label">S级热度</div></div>
        <div class="stat-card" data-accent="cyan"><div class="stat-number" style="color:var(--primary);">${data.total_endorsements || data.endorsements || '-'}</div><div class="stat-label">可查代言</div></div>`;
    } catch (err) {
      // 离线回退：展示示例统计
      ...
      container.innerHTML = `
        <div class="stat-card" data-accent="primary"><div class="stat-number">${demo.length}</div><div class="stat-label">示例艺人</div></div>
        <div class="stat-card" data-accent="safe"><div class="stat-number" style="color:var(--risk-safe);">${low}</div><div class="stat-label">低风险艺人</div></div>
        <div class="stat-card" data-accent="warn"><div class="stat-number" style="color:var(--heat-s);">${s}</div><div class="stat-label">S级热度</div></div>
        <div class="stat-card" data-accent="cyan"><div class="stat-number" style="color:var(--primary);">-</div><div class="stat-label">在线模式查看全部</div></div>`;
    }
    // 风险态势趋势（渐变面积图）
    this.loadRiskTrend();
  },async loadRiskTrend() {
    ...
    let data;
    try {
      data = await this.api('/events/risk-trend?months=12');
    } catch (e) {
      const card = document.getElementById('riskTrendCard');
    ...
    const srcEl = document.getElementById('riskTrendSource');
    if (srcEl && data.note) srcEl.textContent = '数据源：risk_events';
    ...
    if (this.state.riskTrendChart) this.state.riskTrendChart.destroy();
    this.state.riskTrendChart = new Chart(ctx, {
      type: 'line',
      data: {
        labels,
        datasets: [
          {
            ...
            data: total,
            borderColor: '#7C8CFF',
            backgroundColor: grad(ctx, 'rgba(124,140,255,0.42)', 'rgba(124,140,255,0.02)'),
            fill: true, tension: 0.4, borderWidth: 2.5,
            pointRadius: 0, pointHoverRadius: 6, pointBackgroundColor: '#7C8CFF',
            pointHoverBorderColor: '#fff', pointHoverBorderWidth: 2,
            shadowColor: 'rgba(124,140,255,0.6)', shadowBlur: 12,
          },
          {
            label: '高危/严重事件',
            data: high,
            borderColor: '#FF6B8B',
            backgroundColor: grad(ctx, 'rgba(255,107,139,0.34)', 'rgba(255,107,139,0.02)'),
            fill: true, tension: 0.4, borderWidth: 2.5,
            pointRadius: 0, pointHoverRadius: 6, pointBackgroundColor: '#FF6B8B',
            pointHoverBorderColor: '#fff', pointHoverBorderWidth: 2,
          },
        ],
      },
      options: {
        ...
        plugins: {
          legend: { position: 'bottom', labels: { usePointStyle: true, padding: 16, font: { size: 12 }, color: 'rgba(174,182,214,0.9)' } },
          tooltip: {
            backgroundColor: 'rgba(13,19,38,0.95)', titleColor: '#EAF0FF', bodyColor: '#AFB8D8',
            borderColor: 'rgba(124,140,255,0.3)', borderWidth: 1, padding: 12, cornerRadius: 10,
            titleFont: { size: 12 }, bodyFont: { size: 12 },
          },
        },
        scales: {
          y: { beginAtZero: true, grid: { color: 'rgba(130,150,220,0.14)' }, ticks: { color: 'rgba(124,135,172,0.9)', font: { size: 10 } } },
          x: { grid: { display: false }, ticks: { color: 'rgba(124,135,172,0.9)', font: { size: 10 }, maxRotation: 45 } },
        },
      },
    });
    ...
    container.innerHTML = `
      ...
      <div class="page-content">
        ...
        <!-- 平台风险态势 · 渐变面积图 -->
        <div class="card mb-6" id="riskTrendCard">
          <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
            <span>平台风险态势趋势 <span class="text-xs text-tertiary">· 近 12 个月负面事件量（真实数据）</span></span>
            <span id="riskTrendSource" class="tag" style="font-size:11px;padding:2px 8px;background:var(--gray-100,#F3F4F6);color:var(--text-secondary);"></span>
          </div>
          <div class="card-body">
            <div style="max-width:100%;height:280px;position:relative;">
        ...
        <div class="tab-content" id="tab-commercial">
          ...
          <div class="grid-2">
            <div class="card">
              ...
              <div class="card-body">
                <div class="table-responsive"><table class="data-table">
                  <tr><td style="color:var(--text-secondary);">商务报价</td><td style="font-weight:600;color:var(--primary);">${this.maskedInline(a.commercial_quote, '商务报价')}</td></tr>
                  <tr><td style="color:var(--text-secondary);">经纪公司</td><td>${this.maskedInline(a.agency, '经纪公司')}</td></tr>
                  <tr><td style="color:var(--text-secondary);">历史代言</td><td>${this.maskedInline(a.brand_history, '历史代言')}</td></tr>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;">
          ${items.map(x => `
            <div class="card" style="padding:14px;cursor:pointer;border:1px solid var(--border,#E5E7EB);" onclick="App.navigate('artist/${x.artist_id}')">
              ...
              <div style="font-size:12px;color:var(--text-tertiary);margin-top:2px;">${x.heat_level || '-'} · ${this.maskedInline(x.agency, '经纪公司')}</div>
    ...
    this.runSopSearch();
  },async renderBillingPage(container) {
    container.innerHTML = `<div style="text-align:center;padding:40px"><div class="spinner"></div></div>`;
    let catalog, sub = null, usage = null;
    try { catalog = await this.api('/billing/catalog'); } catch (e) { catalog = null; }
    try { sub = await this.api('/billing/subscription'); } catch (e) { /* 未登录 */ }
    try { usage = await this.api('/billing/usage'); } catch (e) { /* 未登录 */ }

    if (!catalog) {
      container.innerHTML = `<div class="card"><div class="card-body text-center"><p style="color:var(--text-tertiary)">计费目录加载失败，请稍后重试</p></div></div>`;
      return;
    }
    const plans = catalog.plans || [];
    const modules = catalog.modules || [];

    // 支付通道可用性（真实微信/支付宝需 Render 注入凭据才会 available）
    let methods = { sandbox: true, wechat: false, alipay: false };
    try { methods = await this.api('/billing/methods'); } catch (e) { /* 忽略 */ }
    if (!this.state.payChannel) this.state.payChannel = 'sandbox';
    const chanDefs = [
      { k: 'sandbox', label: '🧪 演示支付', on: methods.sandbox },
      { k: 'wechat', label: '💚 微信支付', on: methods.wechat },
      { k: 'alipay', label: '🔵 支付宝', on: methods.alipay },
    ];
    const chanHtml = `<div class="pay-channel" style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;">
      <span style="font-size:12px;color:var(--text-tertiary);align-self:center;">支付方式：</span>
      ${chanDefs.map(c => {
        const cls = 'chan-btn ' + (this.state.payChannel === c.k ? 'on ' : '') + (c.on ? '' : 'disabled');
        const attr = c.on ? ('data-ch="' + c.k + '" onclick="App.setPayChannel(\'' + c.k + '\')"') : ('data-ch="' + c.k + '" disabled');
        return '<button class="' + cls + '" ' + attr + '>' + c.label + (c.on ? '' : '（未启用）') + '</button>';
      }).join('')}
    </div>`;

    // 当前套餐高亮
    const curType = (sub && sub.user_type) || this.state.plan || 'free';

    const planCards = plans.map(p => {
      const price = p.price_monthly == null ? '定制' : '¥' + p.price_monthly;
      const yearly = p.price_yearly ? `<span class="billing-yr">¥${p.price_yearly}/年</span>` : (p.price_monthly == null ? '<span class="billing-yr">按席位/用量</span>' : '');
      const isCur = p.plan_type === curType;
      const modNames = (p.modules || []).map(k => { const m = modules.find(x => x.key === k); return m ? m.name : k; });
      return `<div class="billing-plan ${p.plan_type === 'professional' ? 'feat' : ''} ${isCur ? 'cur' : ''}">
        ${p.plan_type === 'professional' ? '<div class="billing-badge">最受欢迎</div>' : ''}
        ${isCur ? '<div class="billing-badge cur">当前套餐</div>' : ''}
        <div class="billing-plan-name">${p.name}</div>
        <div class="billing-plan-tag">${p.tagline || ''}</div>
        <div class="billing-price">${price}<small>${p.price_monthly != null ? '/月' : ''}</small></div>
        ${yearly}
        <div class="billing-level">数据层级：${p.data_level_name}（L${p.data_level}）</div>
        <ul class="billing-mods">${modNames.map(n => `<li>${n}</li>`).join('')}</ul>
        ${isCur ? '<button class="btn btn-ghost" style="width:100%;justify-content:center" disabled>当前使用中</button>'
                : `<button class="btn ${p.plan_type === 'enterprise' ? 'btn-cyan' : 'btn-primary'}" style="width:100%;justify-content:center"
                     onclick="App.checkoutPlan('${p.plan_type}','${p.price_monthly != null ? 'monthly' : 'yearly'}')">${p.price_monthly == null ? '定制报价' : '立即订阅'}</button>`}
      </div>`;
    }).join('');

    // 模块组合（B 端）
    const modRows = modules.map(m => `<tr>
      <td><b>${m.name}</b><div style="font-size:11px;color:var(--text-tertiary)">${m.desc}</div></td>
      <td style="text-align:center">L${m.required_level}</td>
      <td style="text-align:right">¥${m.price_monthly}/月</td>
      <td style="text-align:center">${m.metric ? (m.included_quota === -1 ? '不限' : m.included_quota + ' 含') : '—'}</td>
      <td style="text-align:center"><input type="checkbox" class="bmod" data-key="${m.key}" style="width:18px;height:18px"></td>
    </tr>`).join('');
    ...
    if (usage && usage.usage) {
      const u = usage.usage;
      usageHtml = `<div class="card mt-6"><div class="card-body">
        <h3 style="font-size:16px;font-weight:700;margin-bottom:12px;">📊 本月用量（${usage.period}）</h3>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;">
        ${['api_calls', 'report_exports', 'monitored_artists'].map(k => {
          const it = u[k] || {};
          const pct = it.unlimited ? 100 : Math.min(100, Math.round((it.used / (it.quota || 1)) * 100));
          return `<div style="padding:12px;border:1px solid var(--border,#E5E7EB);border-radius:12px;">
            <div style="font-size:12px;color:var(--text-tertiary)">${it.label}</div>
            <div style="font-size:20px;font-weight:700;margin:4px 0">${it.used}${it.unlimited ? '' : ' / ' + (it.quota === -1 ? '∞' : it.quota)}</div>
            <div class="risk-bar" style="width:100%"><i style="width:${pct}%;background:linear-gradient(90deg,#7C3AED,#06B6D4)"></i></div>
          </div>`;
        }).join('')}
        </div></div></div>`;
    }

    const curSubHtml = sub ? `<div class="card mt-6"><div class="card-body">
      <h3 style="font-size:16px;font-weight:700;margin-bottom:8px;">✅ 我的订阅</h3>
      <div style="font-size:13px;color:var(--text-secondary);line-height:1.9">
        当前套餐：<b>${sub.plan_name}</b> · 数据层级 L${sub.data_level} · 可用模块 ${sub.entitled_modules.length} 项<br>
        ${(sub.subscriptions || []).map(s => `生效：${new Date(s.start_date * 1000).toLocaleDateString()} ~ ${new Date(s.end_date * 1000).toLocaleDateString()}（${s.billing_cycle === 'yearly' ? '年付' : '月付'} ¥${s.price}）`).join('<br>') || ''}
      </div>
      <button class="btn btn-ghost" style="margin-top:10px" onclick="App.cancelSub()">取消订阅（回退免费版）</button>
    </div></div>` : '';

    container.innerHTML = `
      <div class="billing-head">
        <h1 style="font-size:24px;font-weight:800;">订阅与计费</h1>
        <p style="font-size:13px;color:var(--text-tertiary);margin-top:4px;">套餐决定数据层级（脱敏自动生效）· 企业版支持模块自由组合与按量计费</p>
        ${chanHtml}
      </div>
      ...
      <div class="card mt-6"><div class="card-body">
        <h3 style="font-size:16px;font-weight:700;margin-bottom:4px;">🧩 B 端模块化计费（企业/机构自定义组合）</h3>
        <p style="font-size:12px;color:var(--text-tertiary);margin-bottom:12px;">勾选所需模块，系统实时试算月费（年付按 10 个月优惠）。企业版可在此基础叠加席位与定制报价。</p>
        <table class="billing-tbl"><thead><tr><th>模块</th><th style="text-align:center">层级</th><th style="text-align:right">月价</th><th style="text-align:center">包含配额</th><th style="text-align:center">组合</th></tr></thead>
        <tbody>${modRows}</tbody></table>
        <div style="display:flex;align-items:center;gap:16px;margin-top:14px;flex-wrap:wrap">
          <div id="composeResult" style="font-size:14px;font-weight:700;"></div>
          <button class="btn btn-cyan" onclick="App.composeModules()">试算并定制企业方案</button>
        </div>
      </div></div>`;
  },async composeModules() {
    const keys = Array.from(document.querySelectorAll('.bmod:checked')).map(c => c.getAttribute('data-key'));
    if (!keys.length) { this.showToast('请至少勾选 1 个模块', 'info'); return; }
    try {
      const res = await this.api('/billing/compose', { method: 'POST', body: JSON.stringify({ modules: keys.map(k => ({ module_key: k })), billing_cycle: 'monthly' }) });
      const el = document.getElementById('composeResult');
      if (el) el.innerHTML = `组合月费 <span style="color:var(--primary)">¥${res.total_payable}</span>（年付 ¥${res.total_payable * 10 / 10 * 10 / 10 / 1}）`;
      this.showToast('试算完成，可提交企业定制报价', 'success');
    } catch (e) { this.showToast('试算失败：' + (e.message || ''), 'error'); }
  },setPayChannel(ch) {
    this.state.payChannel = ch;
    document.querySelectorAll('.chan-btn').forEach(b => b.classList.toggle('on', b.getAttribute('data-ch') === ch));
  },async cancelSub() {
    if (!confirm('确认取消订阅并回退免费版？')) return;
    try {
      await this.api('/billing/cancel', { method: 'POST', body: JSON.stringify({ reason: 'user' }) });
      this.showToast('已取消，回退免费版', 'success');
      this.route();
    } catch (e) { this.showToast('取消失败：' + (e.message || ''), 'error'); }
    ...
    const draw = () => {
      try {
        ...
        const gridColor = dark ? 'rgba(130,150,220,0.18)' : '#E5E7EB';
        const labelColor = dark ? 'rgba(174,182,214,0.92)' : '#4B5563';
        const tickColor = dark ? 'rgba(124,135,172,0.8)' : '#9CA3AF';
        // 渐变填充（主色 → 青）
        const g = ctx.createLinearGradient(0, 0, 0, canvas.height || 240);
        g.addColorStop(0, 'rgba(124,140,255,0.45)');
          ...
          options: {
            ...
            scales: {
              r: {
                ...
                pointLabels: { font: { size: 13, weight: '500' }, color: labelColor },
            ...
            plugins: {
              ...
              tooltip: {
                backgroundColor: dark ? 'rgba(13,19,38,0.95)' : '#1E293B',
                titleColor: '#EAF0FF', bodyColor: '#AFB8D8',
                borderColor: 'rgba(124,140,255,0.3)', borderWidth: 1, padding: 10, cornerRadius: 10,
                callbacks: {
          ...
          options: {
            ...
            scales: {
              r: {
                ...
                ticks: { stepSize: 25, color: tickColor, backdropColor: 'transparent' },
                grid: { color: gridColor },
                pointLabels: { font: { size: 12, weight: '500' }, color: labelColor },
                angleLines: { color: gridColor },
              }
            },
            plugins: {
              ...
              tooltip: {
                backgroundColor: dark ? 'rgba(13,19,38,0.95)' : '#1E293B',
                titleColor: '#EAF0FF', bodyColor: '#AFB8D8',
                borderColor: 'rgba(167,139,250,0.3)', borderWidth: 1, padding: 10, cornerRadius: 10,
              }
            }
          }
        });
    ...
    try {
      ...
      this.state.gcvChart = new Chart(canvas, {
        ...
        data: { labels, datasets: [{ label: 'GCV 五维', data: scores,
          backgroundColor: g, borderColor: '#7C8CFF', borderWidth: 2.5,
          pointBackgroundColor: '#22D3EE', pointBorderColor: '#fff', pointBorderWidth: 1.5,
          pointRadius: 3, pointHoverRadius: 6 }] },
        options: { responsive: true,
          scales: { r: { beginAtZero: true, max: 100, ticks: { font: { size: 9 }, stepSize: 20, color: tickColor, backdropColor: 'transparent' }, grid: { color: gridColor }, angleLines: { color: gridColor }, pointLabels: { font: { size: 11 }, color: labelColor } } },
  ...
  },closeBrandMatch() { const m = document.getElementById('brandMatchModal'); if (m) m.classList.remove('show'); },switchBrandTab(tab) {
  ...
  },gotoMyRequirements() { this.openBrandMatch(); this.switchBrandTab('my'); },gotoDemandBoard() { this.openBrandMatch(); this.switchBrandTab('board'); },async downloadRiskReport(artistId, artistName) {
    try {
  ...
  },populateWhitelabelTemplates() { this.loadWhitelabelTemplates(); },_wlTplKey() { return 'yiancha_wl_templates'; },loadWhitelabelTemplates() {
    try {
