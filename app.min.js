...
const App = {API_BASE: 'https://yiancha-backend.onrender.com/api/v1',state: {
  ...
  },async checkoutPlan(plan, cycle = 'monthly') {
    ...
    try {
      const res = await this.api('/billing/checkout', {
        method: 'POST',
        body: JSON.stringify({ plan_type: plan, billing_cycle: cycle }),
      });
    ...
    overlay.innerHTML = `
      <div class="modal-card pay-card">
        ...
        <div style="font-size:12px;color:var(--text-tertiary);margin-bottom:14px">
          ${res.sandbox ? '沙箱演示支付（无真实扣款）· 生产将接微信/支付宝' : ''}
        </div>
        <div class="pay-qr"><img src="${qr}" alt="pay qr" /></div>
        <div class="pay-amount">¥${res.pay_amount} <small>元</small></div>
        <div class="pay-meta">订单号 ${res.order_no}</div>
        ${res.pay_url ? `<a class="btn btn-ghost" style="margin:10px 0" href="${res.pay_url}" target="_blank">或打开支付页</a>` : ''}
